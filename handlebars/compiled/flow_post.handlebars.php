...
'.$sp.'' : '').''.((LR::ifvar($cx, (($inary && isset($in['name'])) ? $in['name'] : null), false)) ? ''.LR::encq($cx, LR::hbch($cx, 'avatar', array(array($in),array()), 'encq', $in)).'
'.$sp.'			<bdi>'.LR::encq($cx, (($inary && isset($in['name'])) ? $in['name'] : null)).'</bdi>' : ''.LR::encq($cx, LR::hbch($cx, 'l10n', array(array('flow-anonymous'),array()), 'encq', $in)).'').''.((LR::ifvar($cx, ((isset($in['links']) && is_array($in['links']) && isset($in['links']['userpage'])) ? $in['links']['userpage'] : null), false)) ? ''.LR::encq($cx, LR::hbch($cx, 'title', array(array($in),array()), 'encq', $in)).'</a>' : '').'<span class="mw-usertoollinks flow-pipelist">
